<template>
  <div>
    <div class="designOrder">
      ...
      <div class="listAll">
        <scroller class="supervisionOrder"  
          :on-refresh="refresh"
          :on-infinite="infinite"
          ref="my_scroller">
          <div class="con_list" v-for="(item, index) in supervisionOrders" :key="index">
            <group>
            ...
            <span @click="hrefDetails(item.order_id)" v-if="(item.order_state == '30' && item.refund_list.length == '0')">确认</span>
            <router-link :to="{ path: 'supervisionOrderEva', query: { order_id: item.order_id }}" v-if="(item.order_state=='40' && item.evaluation_state=='0')">评价</router-link>
            <router-link :to="{ path: 'supervisionRefundDetails', query: { order_id: item.order_id }}" v-if="(item.refund_list.length>0)">退款详情</router-link>
            <router-link :to="{ path: 'supervisionRefundApply', query: { order_id: item.order_id }}" v-if="(item.order_state=='30' && item.lock_state == '0' || item.order_state=='20' && item.lock_state == '0')">申请退款</router-link>
          </div>
        </scroller>
      </div>
    </div>
...
<script>
  ...
  import {supervisionOrder} from '../../api/api'
  ...
  export default {
    ...
    methods: {
      ...
      _supervisionOrder () {
        supervisionOrder(this.type, this.curpage).then((res) => {
          ...
          for(var i in res.data.items){
            ...
            this.supervisionOrders.push(res.data.items[i])
          }
        })
      },
      order_status (num) {
...
<style type="text/css" scoped>
  ...
  .supervisionOrder{background-color: #fff;margin-bottom: 5rem;position: relative!important;height: calc(100vh - 9rem)!important;}
