  ...
  <div class="orderDetails">
    ...
    <div class="order cons">
    	...
    	<p class="cons_p">
    		...
    		<span class="order_r">{{designOrderDetails.order_sn}}</span>
    	</p>
    	<p class="cons_p">
    		<span class="order_l">下单时间</span>
    		<span class="order_r">{{designOrderDetails.createTime}}</span>
    	</p>
    	<p class="cons_p">
    		<span class="order_l">支付方式</span>
    		<span class="order_r">{{designOrderDetails.payment_name}}</span>
    	</p>
    	<p class="cons_p">
    		<span class="order_l">实付款</span>
    		<span class="order_r">￥{{designOrderDetails.pay_amount}}</span>
    	</p>
    </div>
    <div class="zx cons">
    	...
    	<p class="cons_p">
    		...
    		<span class="zx_r">{{designOrderDetails.is_price}}</span>
    	</p>
    	<p class="cons_p">
    ...
    <div class="service cons">
        ...
        <span class="service_r" v-if="item.item_state == '30'">不通过</span>  
    		<span class="service_r" v-if="item.item_state == '20'">已确认</span>        
        <span class="service_r" v-if="item.item_state == '10'">
          <span class="btn done" @click="service_submit(item.order_item_id, 2, designOrderDetails.order_id)">不通过</span>
          <span class="btn do" @click="service_submit(item.order_item_id, 1, designOrderDetails.order_id)">通过</span>
        </span>        
...
<script>
  ...
  import {BASEURL, designOrderDetails, designServiceSubmit} from '../../api/api'
  import qs from 'qs'
  ...
  export default {
    ...
    created() {
      ...
      this._designOrderDetails()
    },
    methods: {        
      _designOrderDetails () {
        designOrderDetails(this.order_id).then((res) => {
          this.icon_img(res.data.items.order_info.order_state)
          this.designOrderDetails = res.data.items.order_info
          this.designer = this.designOrderDetails.extend_designer
          this.demand = this.designOrderDetails.extend_demand
          this.service_items = this.designOrderDetails.extend_item
          this.designOrderDetails.is_price = this.is_price(this.designOrderDetails.order_type)
          this.designOrderDetails.createTime = this.timeStamp(this.designOrderDetails.create_time)
          this.demand.is_newHouse = this.is_newHouse(this.demand.house_condition)
          for(var i in this.designOrderDetails.extend_item_submit){
            this.designOrderDetails.extend_item_submit[i].createTime = this.timeStamp(this.designOrderDetails.extend_item_submit[i].create_time)
          }
          this.extend_item_submit = this.designOrderDetails.extend_item_submit
        })
      },
      timeStamp (value) {
        function add0(m) {
      ...
      service_submit (item_id, type, order_id) {
        ...
        var _this = this
        designServiceSubmit(qs.stringify(jsonData)).then((res) => {
          AlertModule.show({
            title: '系统提示',
