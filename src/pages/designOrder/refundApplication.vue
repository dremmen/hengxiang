	...
	<div class="designOrderEva">
		<div class="designOrderEva_con">
			...
			<div class="orderDetails_con">
				...
				<p class="cons">
					<span class="style">风格：{{extend_demand.style}}</span>
		...
		<span class="btn" @click="_refundSubmit()">提交</span>
	</div>
</template>

<script type="text/javascript">
	import { Rater, XTextarea, PopupRadio, AlertModule } from 'vux'
	import { refund_reason, designOrderDetails, refundSubmit, evaluate } from '../../api/api'
	import qs from 'qs'
      ...
      close (index) {
      	...
      	console.log(this.refund_price)
	  		if(this.refund_price <= this.yes_refund_price && this.refund_price<0){
	  			...
	  			refundSubmit(qs.stringify(jsonData)).then((res) => {
	  				if(res.data.status = "true"){
	  					AlertModule.show({
			          title: '系统提示',
			          content: res.data.msg,
			          onHide () {
			          	_this.$router.push({path: 'refundDetails', query:{'order_id': order_id}})
			          }
			        })	
            }else{
              AlertModule.show({
